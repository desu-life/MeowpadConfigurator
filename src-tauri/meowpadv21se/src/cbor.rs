...
pub struct Keyboard {
    ...
    #[serde(rename = "sb")]
    pub SideBtnKeyConfigs: [NormalKeyConfig; 3],
    #[serde(rename = "jet")]
...
impl From<config::HallKeyConfig> for KeyRTConfig {
    fn from(cfg: config::HallKeyConfig) -> Self {
        Self { PressPercentage: cfg.press_percentage, ReleasePercentage: cfg.release_percentage, DeadZone: cfg.dead_zone, KeyData: cfg.key_data.into_iter().collect::<KbReport>().into() }
    }
}

impl From<config::NormalKeyConfig> for NormalKeyConfig {
    fn from(cfg: config::NormalKeyConfig) -> Self {
        Self { KeyData: cfg.key_data.into_iter().collect::<KbReport>().into() }
    }
}
...
impl From<config::Key> for Keyboard {
    fn from(cfg: config::Key) -> Self {
        ...
        for i in 0..3 {
            key_configs[i] = cfg.hall_keys[i].into();
        }

        let mut side_btn_key_configs = [NormalKeyConfig::default(); 3];
        for i in 0..3 {
            side_btn_key_configs[i] = cfg.normal_keys[i].into();
        }
...
impl Default for Keyboard {
    fn default() -> Self {
        ...
        let side_btn_key_configs = [
            NormalKeyConfig::with_key(KeyCode::Escape),
            NormalKeyConfig::with_key(KeyCode::F10),
            NormalKeyConfig::with_key(KeyCode::F11),
        ];
